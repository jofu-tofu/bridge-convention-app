...
13. **Static HTML for SEO, WASM for interaction.** Learn pages are pre-rendered as static HTML at build time for search engine discoverability; a Rust binary (`bridge-static`) extracts viewport JSON from `bridge-session` into `.generated/learn-data.json`, and SvelteKit content routes prerender the final `/learn/*` pages from that data. The WASM app is not hydrated on static pages — they are standalone, no-JS documents. The SPA is one click away via practice CTAs.
