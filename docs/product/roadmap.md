...
4. **Deal Review** — surface a browsable table of past deals so users can revisit hands they played, with filters (convention, role, system, outcome, date). Today drill state is ephemeral; nothing persists per-deal history. Blocked on: (a) DataPort schema for per-deal records, (b) viewport snapshot/serialization decision (full hand replay vs summary), (c) UI design spec for the table + filter chrome.
