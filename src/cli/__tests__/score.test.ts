...
describe("score command", () => {
  it("calculates basic 3NT making score", async () => {
    const result: CommandResult = await scoreCommand.handler(
      { contract: "3NT", tricks: "9" },
      deps,
    );
    expect(result.type).toBe("score");
    const data = result.data as { score: number; contract: string };
  ...
  it("calculates vulnerable score", async () => {
    ...
    expect(result.type).toBe("score");
    const data = result.data as { score: number; vulnerable: boolean };
  ...
  it("throws on invalid contract string", async () => {
    await expect(
      scoreCommand.handler({ contract: "8NT", tricks: "9" }, deps),
    ).rejects.toMatchObject({ code: "INVALID_ARGS" });
  });

  it("throws when missing contract", async () => {
    await expect(
      scoreCommand.handler({ tricks: "9" }, deps),
    ).rejects.toMatchObject({ code: "INVALID_ARGS" });
  });

  it("throws when missing tricks", async () => {
    await expect(
      scoreCommand.handler({ contract: "3NT" }, deps),
    ).rejects.toMatchObject({ code: "INVALID_ARGS" });
  });
