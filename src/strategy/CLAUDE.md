...
strategy/
  bidding/
    ...
    protocol-adapter.ts      protocolSpecToStrategy() — ConventionSpec → ConventionStrategy. Rule-only path: buildObservationLogViaRules() with per-step kernel threading, no FSM replay. All bundles use ruleModules (Phase 5 complete). Phase 6: incremental local FSM phase caching in `buildObservationLogViaRules()` — O(N×M) instead of O(N²×M). Exports buildObservationLogViaRules(), findMatchingClaimForCall().
