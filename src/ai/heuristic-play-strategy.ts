...
const openingLeadHeuristic: PlayHeuristic = {
  ...
  apply(context: PlayContext): Card | null {
    ...
    if (!isNT && context.trumpSuit !== undefined) {
      for (const suit of Object.keys(suitGroups) as Suit[]) {
        ...
        if (hasAce && hasKing) {
          ...
          if (
            legalPlays.some(
              (c) => c.suit === ace.suit && c.rank === ace.rank,
            )
          ) {
            return ace;
          }
    ...
    for (const suit of Object.keys(suitGroups) as Suit[]) {
      ...
      if (
        top &&
        legalPlays.some((c) => c.suit === top.suit && c.rank === top.rank)
      ) {
        return top;
      }
    }
    ...
    if (isNT) {
      ...
      if (longestSuit && longestLen >= 4) {
        ...
        if (
          fourthBest &&
          legalPlays.some(
            (c) => c.suit === fourthBest.suit && c.rank === fourthBest.rank,
          )
        ) {
          return fourthBest;
        }
    ...
    if (!isNT) {
      for (const suit of Object.keys(suitGroups) as Suit[]) {
        if (suit === context.trumpSuit) continue; // Don't lead singleton trump
        const cards = suitGroups[suit]!;
        if (cards.length === 1) {
          const card = cards[0]!;
          if (
    ...
    // Avoid leading unsupported honors — pick lowest from longest non-honor-only suit
