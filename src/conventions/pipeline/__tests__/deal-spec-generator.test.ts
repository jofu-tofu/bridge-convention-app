...
function makeMinimalSpec(overrides: Partial<DealSpec> = {}): DealSpec {
  return {
    specId: "test-spec",
...
describe("compileDealSpec", () => {
  describe("role-relative seat mapping", () => {
    ...
    it("maps lho and rho correctly from East perspective", () => {
      ...
      const { dealConstraints } = compileDealSpec(spec, Seat.East);
  ...
  describe("joint constraints", () => {
    it("produces a diagnostic for joint constraints", () => {
      ...
      const { diagnostics } = compileDealSpec(spec, Seat.South);
...
describe("generateDealSpec", () => {
  it("generates a deal satisfying basic seat constraints", () => {
    ...
    const result = generateDealSpec(spec, Seat.South);
  ...
  describe("diagnostic mode", () => {
    it("returns unsat for impossible HCP (all seats want 15+)", () => {
      ...
      const unsat = result as UnsatisfiableResult;
