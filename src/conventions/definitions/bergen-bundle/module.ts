 ...
 *
 * Contains all Bergen-specific bidding logic, surfaces, post-R1 machine states,
 * facts, explanation entries, and pedagogical relations. The R1 transitions are
 * in the skeleton (compose.ts) because they are suit-dependent; this module
 * contributes only the post-R1 subtree and all surface groups.
 */
...
// ─── Post-R1 machine states (R2, R3, R4) ────────────────────

const BERGEN_MODULE_STATES: readonly MachineState[] = [
  // ─── R2: opener rebids ──────────────────────────────────────
...
export const bergenModule: ConventionModule = {
  ...
  // No single entry point — Bergen dispatches per suit from skeleton states.
  // R1 surfaces are provided as surfaceGroups below, matched by machine surfaceGroupId.
  entrySurfaces: [],
  entryTransitions: [],

  surfaceGroups: [
    // R1: responder initial bids (per suit)
  ...
  machineStates: BERGEN_MODULE_STATES,
