/**
 * WitnessSpecIR runtime — compiles a WitnessSpecIR into engine-level
 * DealConstraints for hand-first rejection-sampling deal generation.
 *
 * The main entry point is {@link compileWitnessSpec}, which converts the
 * declarative IR layers into concrete seat constraints that the existing
 * {@link generateDeal} engine can consume directly.
 *
 * A higher-level {@link generateWitnessSpec} orchestrates compilation,
...
/**
 * Compile a {@link HandPredicateIR} into an engine {@link SeatConstraint}.
 *
 * Recognised fact IDs:
 * - `hcp`       → minHcp / maxHcp
...
/**
 * Compile a {@link WitnessSpecIR} into engine-level {@link DealConstraints}.
 *
...
export function compileWitnessSpecFull(
  ...
  return compileWitnessSpec(spec, userSeat) as CompileResult;
}
 ...
 * {@link WitnessUnsatResult} describing the likely unsat core.
 */
export function generateWitnessSpec(
  spec: WitnessSpecIR,
  userSeat: Seat,
  rng?: () => number,
): WitnessGeneratorResult | WitnessUnsatResult {
