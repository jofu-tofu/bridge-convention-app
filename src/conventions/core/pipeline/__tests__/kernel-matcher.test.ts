...
function kernel(overrides: Partial<KernelState> = {}): KernelState {
  return { ...INITIAL_KERNEL, ...overrides };
}
...
describe("matchKernel", () => {
  describe("fit / no-fit", () => {
    ...
    it("fit() does not match null fitAgreed", () => {
      expect(matchKernel({ kind: "fit" }, INITIAL_KERNEL)).toBe(false);
    });

    it("no-fit matches null fitAgreed", () => {
      expect(matchKernel({ kind: "no-fit" }, INITIAL_KERNEL)).toBe(true);
    });
  ...
  describe("forcing", () => {
    ...
    it("matches none forcing on initial kernel", () => {
      expect(matchKernel({ kind: "forcing", level: "none" }, INITIAL_KERNEL)).toBe(true);
    });
  });
  ...
  describe("captain", () => {
    ...
    it("matches undecided on initial kernel", () => {
      expect(matchKernel({ kind: "captain", who: "undecided" }, INITIAL_KERNEL)).toBe(true);
    });
  });

  describe("competition", () => {
    it("uncontested matches initial kernel", () => {
      expect(matchKernel({ kind: "uncontested" }, INITIAL_KERNEL)).toBe(true);
    });
    ...
    it("overcalled does not match uncontested", () => {
      expect(matchKernel({ kind: "overcalled" }, INITIAL_KERNEL)).toBe(false);
    });
    ...
    it("doubled matches doubled competition", () => {
      ...
      expect(matchKernel({ kind: "doubled" }, INITIAL_KERNEL)).toBe(false);
    });
  ...
  describe("combinators", () => {
    it("and requires all to match", () => {
      ...
      expect(matchKernel(expr, INITIAL_KERNEL)).toBe(true);
