...
describe("extractKernelState", () => {
  it("maps default registers to INITIAL_KERNEL", () => {
    const result = extractKernelState(makeRegisters());
    expect(result).toEqual(INITIAL_KERNEL);
  });
...
describe("computeKernelDelta", () => {
  it("returns empty object for identical kernels", () => {
    const delta = computeKernelDelta(INITIAL_KERNEL, INITIAL_KERNEL);
    expect(delta).toEqual({});
  });

  it("returns only changed fields", () => {
    const before = INITIAL_KERNEL;
    const after: KernelState = {
      ...INITIAL_KERNEL,
      forcing: "game",
      captain: "responder",
    };
