...
**`createIntentBidFactory(prefix)`** generates deterministic `prefix/name` nodeIds. Duplicates within a factory throw at construction. Stayman/Bergen/Weak Twos/SAYC use factories; Lebensohl Lite uses `createIntentBidFactory()` (migrated from deprecated `intentBid()`).
