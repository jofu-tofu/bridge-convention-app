...
- **Factory + DI.** `createGameStore(service, { getActiveLaunch? })` takes `DevServicePort` plus an optional active-launch callback for multi-module drill rotation. Tests inject a stub engine via `createLocalService(engine)`. No `vi.mock`.
...
| File                 | Role                                                                                               |
| -------------------- | -------------------------------------------------------------------------------------------------- |
| `app.svelte.ts`      | `createAppStore()` — selected convention, coverage state, dev seed, dev flags (`autoplay`, `debugExpanded`, `autoDismissFeedback`, `skipToPhase`), drill tuning (persisted to localStorage), and the `/practice` launch surface. `practiceRole` in `bridge-app:practice-preferences` now persists as `PracticeRole | "auto"` with `"auto"` as the default. `applyDrillSession(config, conventions)` writes session-scoped mode/role/system + the four per-drill gameplay tunables (`opponentMode`, `playProfileId`, `vulnerabilityDistribution`, `showEducationalAnnotations`) into in-memory state via private `setSession*` helpers without calling `savePreferences`, and tracks the last launch via `activeLaunch`. Screen navigation handled by SvelteKit file-based routing (`goto()` from `$app/navigation`), not store-driven. No `Screen` type or `navigateToX` methods. |
| `game.svelte.ts`     | `createGameStore(service, { getActiveLaunch? })` — coordinator/facade, drill lifecycle, user action handlers, thin reactive cache over service viewports. When `activeLaunch.moduleIds.length > 1`, `launchDealIndex` round-robins `currentModuleId` across deals and `startNewDrill()` rebuilds the session config with that module as both `conventionId` and `target` (`{ kind: "module", moduleId }`). Delegates to sub-modules for bidding, play, phase transitions, viewport caching, and DDS |
...
| `drills.svelte.ts` | `createDrillsStore({ defaultSystemId, seedFromPrefs, auth?, dataPort? })` — unified saved-drill store. **Dual-branch:** anonymous (auth.user === null) reads/writes localStorage `bridge-app:drills`; authenticated (auth.user !== null) reads/writes via DataPort drill methods. Schema `Drill{id,name,moduleIds[],practiceMode,practiceRole|"auto",systemSelectionId,opponentMode,playProfileId,vulnerabilityDistribution,showEducationalAnnotations,createdAt,updatedAt,lastUsedAt}`. Public methods `create/update/delete/rename/markLaunched/refresh` return `Promise`; in anonymous mode they apply synchronously and the promise resolves immediately. **Authenticated branch never writes the localStorage key.** Public getters `loadStatus: "loading" \| "anonymous-ready" \| "authenticated-ready" \| "auth-load-error"`, `isSaving`, and `mutationsDisabled` drive UI states. Internal `authGeneration` token discards late server responses across signed-out↔signed-in transitions so they cannot leak data across accounts. On logout, the pre-login anonymous snapshot is restored (no merge). Legacy migration (`bridge-app:drill-presets` etc.) still runs on first anonymous read. The `seedFromPrefs` snapshot is the *one* point at which `drillsStore` reads `appStore.prefs` — used for one-time legacy-record healing only. |
...
| `types.ts`           | Store facade types for context DI consumers. `GameStore` stays explicit; `DrillLaunchConfig` defines the cross-screen launch shape used by `appStore.applyDrillSession()`. |

**Game store key methods:** `startNewDrill`, `startDrillFromHandle`, `userBid`, `retryBid`, `dismissFeedback`, `getExpectedBid` (bidding); `acceptPrompt()`, `declinePrompt()` (declarer prompt — route through `acceptPromptAction`/`declinePromptAction`); `userPlayCard`, `skipToReview`, `skipToPhase`, `autoPlayHand`, `playThisHand`, `restartPlay` (play); `dismissDrillError` clears `drillError`. `startNewDrill` retries once with a fresh seed when the WASM layer surfaces `DealGenerationExhaustedError` (lifted via `isDealGenerationExhausted`); on second exhaustion it sets `drillError = { kind: "dealGenerationExhausted" }` and `GameScreen` renders `DrillErrorModal`. `autoPlayHand` is dev-only — it loops `service.playCard` for user-controlled seats using the first legal play, skips animation, and dispatches `PLAY_COMPLETE` to land in EXPLANATION with the full play log captured. `currentModuleId` exposes the active module chosen for the current drill deal when `activeLaunch` is present. **Internally**, all phase transitions route through `executeTransition(handle, event)` which enforces the pipeline: service actions → viewport fetch → phase transition → side effects. `dispatchPlayTransition` wraps `executeTransition` + AI play animation for events that produce `aiPlays`. See `game.svelte.ts` for signatures.

**Key state:** `effectiveUserSeat` — defaults to South, set to North on declarer swap. Reset by `startDrillFromHandle()`. `isProcessing` + `play.aborted` flags guard animation races. `transitioning` flag (lifecycle guard) prevents concurrent lifecycle actions. `practiceMode` and `playPreference` are set from `DrillStartResult` and control phase transitions. `launchDealIndex` tracks which module in `activeLaunch.moduleIds` should be used for the next created session; a new `activeLaunch` identity resets it to 0, and subsequent `startNewDrill()` calls increment it round-robin. `handleAutoPromptTransition()` auto-transitions from DECLARER_PROMPT based on `playPreference` (`skip` → EXPLANATION, `always` → PLAYING). `sessionStats` — in-memory per-bid tracking (`correct`, `incorrect`, `streak`), resets on `resetImpl()`. Tracks first attempts only (retries via `isRetryAttempt` flag are skipped). Lives in game store (not a separate store) because it's only 3 fields — extract to `session.svelte.ts` if this grows to per-convention breakdown or persistence.
...
## Per-drill tuning invariants

Two load-bearing rules — do not weaken without explicit product sign-off.

1. **Source-of-truth routing.** When a drill is launched from a saved `Drill`, the `Drill` record is the exclusive source of its four gameplay fields (`opponentMode`, `playProfileId`, `vulnerabilityDistribution`, `showEducationalAnnotations`). When launched from the convention picker (Quick Practice), `appStore.prefs.drill` is the source. `applyDrillSession` writes session-scoped values either way; `GameScreen.svelte` must read them through `appStore`'s session-aware getters — **never directly from `prefs`**.

2. **No-inheritance invariant.** `drillsStore` reads `appStore.prefs` exactly once (at construction, via the injected `seedFromPrefs` snapshot) for the purpose of one-time legacy-record healing. After that, drills carry fully explicit values. Do not add "fall back to prefs if missing" logic in `create`, `update`, `applyDrillSession`, or `GameScreen`. If a missing field is observed at runtime, that's a bug — fail loud, don't paper over it.

**Known trade-off (deliberate):** bulk-editing gameplay fields across many saved drills requires editing each drill individually. This is the accepted cost of "explicit always, no inheritance" over a CSS-cascade-style alternative. Revisit only with product sign-off.
...
<!-- context-layer: generated=2026-02-21 | last-audited=2026-04-18 | version=20 | dir-commits-at-audit=15 | tree-sig=dirs:2,files:21+,exts:ts:20+,md:1 -->
