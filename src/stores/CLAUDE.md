...
**Exported types:** `BidFeedback` (with `grade: BidGrade`, `teachingResolution: TeachingResolution | null`, `practicalRecommendation?: PracticalRecommendation` sourced from `ConventionBiddingStrategy.getLastPracticalRecommendation()`, `teachingProjection?: TeachingProjection` sourced from `ConventionBiddingStrategy.getLastTeachingProjection()`, not from `BidResult`, `encodingTrace?: EncodingTrace`). `resolveTeachingAnswer()` is called with `intentFamilies` from `conventionStrategy.getIntentFamilies()`, `BidGrade`, `TeachingResolution` (re-exported from `teaching/teaching-resolution`), `BidHistoryEntry` (re-exported from `contracts/`, now includes `teachingProjection?: TeachingProjection` for review phase), `GamePhase`, `PlayLogEntry`, `seatController()`.
