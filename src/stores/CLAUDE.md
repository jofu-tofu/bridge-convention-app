...
| `game.svelte.ts`     | `createGameStore(service, { getActiveLaunch? })` — coordinator/facade, drill lifecycle, user action handlers, thin reactive cache over service viewports. When `activeLaunch.moduleIds.length > 1`, `launchDealIndex` round-robins `currentModuleId` across deals and `startNewDrill()` rebuilds the session config with that module as both `conventionId` and `targetModuleId`. Delegates to sub-modules for bidding, play, phase transitions, viewport caching, and DDS |
...
**Game store key methods:** `startNewDrill`, `startDrillFromHandle`, `userBid`, `retryBid`, `dismissFeedback`, `getExpectedBid` (bidding); `acceptPrompt()`, `declinePrompt()` (declarer prompt — route through `acceptPromptAction`/`declinePromptAction`); `userPlayCard`, `skipToReview`, `skipToPhase`, `autoPlayHand`, `playThisHand`, `restartPlay` (play). `autoPlayHand` is dev-only — it loops `service.playCard` for user-controlled seats using the first legal play, skips animation, and dispatches `PLAY_COMPLETE` to land in EXPLANATION with the full play log captured. `currentModuleId` exposes the active module chosen for the current drill deal when `activeLaunch` is present. **Internally**, all phase transitions route through `executeTransition(handle, event)` which enforces the pipeline: service actions → viewport fetch → phase transition → side effects. `dispatchPlayTransition` wraps `executeTransition` + AI play animation for events that produce `aiPlays`. See `game.svelte.ts` for signatures.
