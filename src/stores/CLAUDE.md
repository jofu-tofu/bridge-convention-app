...
| `drills.svelte.ts` | `createDrillsStore({ defaultSystemId, seedFromPrefs })` — unified saved-drill store replacing presets/custom-drills/practice-packs. Schema is `Drill{id,name,moduleIds[],practiceMode,practiceRole|\"auto\",systemSelectionId,opponentMode,playProfileId,vulnerabilityDistribution,showEducationalAnnotations,createdAt,updatedAt,lastUsedAt}` persisted at `bridge-app:drills`. MRU sort is `lastUsedAt` DESC (nulls last) with `updatedAt` DESC tiebreaker. On first load, migrates the three legacy keys read-only, heals IDs via `canonicalBundleId()`, skips malformed legacy records with `console.warn`, and exposes a `migrationSkipped` getter for diagnostics. The `seedFromPrefs` snapshot is the *one* point at which `drillsStore` reads `appStore.prefs` — used to fill the four gameplay tunables on legacy drill records that pre-date them. After construction, drills carry fully explicit values; the store never reads `appStore` again. |
