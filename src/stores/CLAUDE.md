...
**Game store key methods:** `startNewDrill`, `startDrillFromHandle`, `userBid`, `retryBid`, `getExpectedBid`, `getDebugSnapshot` (bidding); `acceptPlay(seatOverride?)`, `declinePlay()`, `acceptPrompt()`, `declinePrompt()` (declarer prompt); `userPlayCard`, `skipToReview`, `skipToPhase`, `playThisHand` (play). `restartPlay` maps to `service.acceptPrompt(handle, "restart")`. `capturePlayInferences` is no longer a separate call — inference data arrives via `BidSubmitResult`/`DrillStartResult` return objects. See `game.svelte.ts` for signatures.
