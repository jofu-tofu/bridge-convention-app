 ...
 * coordinator.
 *
 * MC+DDS play runs entirely in Rust/WASM. The DDS Worker callback is injected
 * at service init. Store calls `playCard` and receives all AI plays regardless
 * of profile.
 */
...
export interface PlayDeps {
  ...
  dispatchEvent: (handle: DrillHandle, event: PhaseEvent) => Promise<unknown>;
...
export function createPlayPhase(deps: PlayDeps) {
  ...
  /**
   * Play a user card via the service. The service handles both heuristic
   * and MC+DDS profiles internally — the store sees no difference.
   */
