...
| `teaching-projection-builder.ts` | `projectTeaching(result: PipelineResult, options?)` — primary overload, builds read-only `TeachingProjection` from `PipelineResult`. Legacy `(arbitration, provenance, options?)` overload exists for backwards compatibility. Pure function, no side effects. Produces `CallProjection[]` (with `projectionKind` classification), `MeaningView[]`, `WhyNotEntry[]` (with optional `familyRelation` from pedagogical graph), `ConventionContribution[]`, `ExplanationNode[]`, `HandSpaceSummary`. All types imported from `core/contracts/`. |
