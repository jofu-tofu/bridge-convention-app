...
components/
  ...
  shared/
    ...
    DrillErrorModal.svelte           Native <dialog> shown by `GameScreen` when `gameStore.drillError` is set. Surfaces `DealGenerationExhausted` after the store's automatic retry. Props: `onDismiss`, optional `onTryAgain`. Self-opens via `$effect` on mount. Test IDs: `drill-error-modal`, `drill-error-modal-retry`, `drill-error-modal-dismiss`.
