...
<div class="bg-yellow-900/90 text-yellow-100 text-xs font-mono px-3 py-2 rounded border border-dashed border-yellow-500/60 shadow-lg">
  ...
  {#if suggestion}
    ...
    {#if suggestion.explanation}
      <div class="mt-1 text-yellow-200/60 leading-tight">{suggestion.explanation}</div>
    {/if}
  {:else}
