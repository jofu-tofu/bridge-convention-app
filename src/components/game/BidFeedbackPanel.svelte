...
{#if feedback.isCorrect}
  ...
  <div class="rounded-lg border-2 border-green-500/60 bg-green-950/80 px-4 py-3 text-center" role="alert">
    <p class="text-green-300 font-semibold text-sm">Correct!</p>
    <p class="text-green-400 font-mono text-lg mt-1">{formatCall(feedback.userCall)}</p>
    {#if feedback.expectedResult?.ruleName}
      <p class="text-green-300/70 text-xs mt-1">{feedback.expectedResult.ruleName}</p>
    {/if}
  </div>
{:else}
  <!-- Wrong bid — red feedback -->
  <div class="rounded-lg border-2 border-red-500/60 bg-red-950/80 px-4 py-3" role="alert">
    <div class="flex items-center gap-2 mb-2">
      <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="text-red-400" aria-hidden="true"><circle cx="12" cy="12" r="10"/><line x1="15" y1="9" x2="9" y2="15"/><line x1="9" y1="9" x2="15" y2="15"/></svg>
      <span class="text-red-300 font-semibold text-sm">Incorrect Bid</span>
    </div>

    <p class="text-red-200/80 text-xs mb-3">
      You bid <span class="font-mono font-bold text-red-200">{formatCall(feedback.userCall)}</span>
    </p>

    {#if showAnswer && feedback.expectedResult}
      <div class="bg-red-900/50 rounded px-3 py-2 mb-3 border border-red-500/30">
        <p class="text-xs text-red-300/70 mb-1">Correct bid:</p>
        <p class="font-mono font-bold text-base text-red-100">{formatCall(feedback.expectedResult.call)}</p>
        {#if feedback.expectedResult.ruleName}
    ...
    <div class="flex flex-col gap-2">
      {#if !showAnswer}
        <Button variant="secondary" onclick={() => { showAnswer = true; }}>
          Show Answer
        </Button>
      {/if}
      ...
      <Button variant="ghost" onclick={onSkipToReview}>
        Skip to Review
      </Button>
    </div>
  </div>
