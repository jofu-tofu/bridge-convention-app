...
<DebugSection
  ...
  preview={pipelineResult?.selected ? fmtCall(pipelineResult.selected.call) + " — " + (pipelineResult.selected.proposal.teachingLabel?.name ?? pipelineResult.selected.proposal.meaningId) : pipelineResult ? "no match (pass)" : null}
>
  {#if pipelineResult}
