<script lang="ts">
  ...
  import { generateCoverageManifest } from "../../conventions/core/runtime/coverage-spec-compiler";
  import type { CoverageManifest, CoverageTarget } from "../../conventions/core/runtime/coverage-spec-compiler";

  const appStore = getAppStore();

  // Generate manifests for all bundles on mount
  ...
  function targetUrl(bundleId: string, stateId: string): string {
    const base = window.location.origin + window.location.pathname;
    return `${base}?convention=${bundleId}&targetState=${encodeURIComponent(stateId)}`;
  }
