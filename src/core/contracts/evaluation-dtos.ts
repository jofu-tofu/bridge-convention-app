...
export interface CandidateEligibility {
  ...
  readonly protocol: {
    readonly satisfied: boolean;
    /** Reason strings fall into two semantic categories:
     *  - `"Suppressed by overlay: {id}"` — context not permitted (overlay state)
     *  - `"Resolver declined: {type}"` — agreement not available (resolver logic)
     *  Consumers needing to distinguish can pattern-match on these prefixes. */
...
export interface EvaluationTrace {
  ...
  /** Number of Monte Carlo samples used by the posterior engine. */
  readonly posteriorSampleCount?: number;
  /** Average confidence across posterior facts [0,1]. */
