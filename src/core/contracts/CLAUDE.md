...
- **Alertability is auto-derived from disclosure; constraints carry isPublic directly.** Alertability is derived from the `disclosure` field on `BidMeaning` — natural disclosure produces no alert; everything else defaults to conventional (alertable). The pipeline derives alertability in `resolveAlert()`. Constraints carry their own `isPublic` flag on the `FactConstraint` itself — consumers filter by `isPublic` as needed. Convention authors write clauses with `isPublic: true` on facts worth disclosing; the framework threads `isPublic` through the entire pipeline. See `alert.ts` for `isAlertable()` and `resolveAlert()`.
...
| **Public-state vocabulary** | `contracts/bidding.ts` (`ForcingState`, `BidAlert`). `pipeline/alert.ts` (`resolveAlert()`, `isAlertable()`). Machine registers in `conventions/core/runtime/machine-types.ts`. | Built-in registers (`force`, `obligation`, `agreement`, `competition`, `captaincy`) managed by conversation machine. |
