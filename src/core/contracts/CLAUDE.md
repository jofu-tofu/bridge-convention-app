...
| `meaning-surface.ts` | `MeaningSurfaceClause`, `MeaningSurface` (with optional `alert: "alert" \| "announce"`, `closurePolicy`, `denies`) — authored, hand-independent meaning units (pre-evaluation). Public constraints auto-derived from primitive/bridge-observable clauses via `derivePublicConstraints()`. Evaluated by `conventions/core/pipeline/meaning-evaluator.ts` into `MeaningProposal`. |
...
| `agreement-module.ts` | `ActivationKind`, `ModuleKind`, `PriorityClass`, `DecisionSurfaceIR`, `AttachmentIR`, `FactConstraintIR`, `PublicConsequences` (deprecated), `ChoiceClosurePolicy`, `ClosureDomain`, `SystemProfileIR`, `ModuleEntryIR`, `ConflictPolicyIR`, `DeclaredEncoderKind`, `PublicEvent`, `PublicConstraint` — agreement module IR types |
