...
| `tree-evaluation.ts` | `ConditionDetail`, `TreePathEntry`, `TreeForkPoint`, `SiblingConditionDetail` (with optional `conditionRole`: `"semantic"` / `"inferential"` / `"pedagogical"` / `"routing"`), `SiblingBid`, `CandidateBid`, `CandidateEligibility`, `ResolvedCandidateDTO` (with optional `allEncodings`), `AlternativeGroup`, `IntentFamily`, `IntentRelationship`, `DecisionTrace`, `CandidateSet`, `EvaluationTrace` (with optional `overlayRescue`, `bundleActiveConventions`, `bundleWinningConvention`), `isDtoSelectable()`, `isDtoPedagogicallyAcceptable()` |
| `evaluation-dtos.ts` | Backward-compat re-export of `tree-evaluation.ts`. All types consolidated into `tree-evaluation.ts`; this file exists only so existing imports don't break. |
...
| `module-surface.ts` | `PublicSnapshot` (with epistemic layers: `publicRecord`, `publicCommitments`, `publicBeliefs`, `latentBranches` — populated by runtime's `buildSnapshotFromAuction()`), `ModuleSurface`, `MultiModuleSurface`, `EncodedProposal`, `EliminationRecord`, `ArbitrationResult`, `SurfaceEvaluationResult`, `TransformApplication`, `SurfaceCompositionDiagnostic`, `buildPublicSnapshot()` |
...
| `teaching-projection.ts` | `TeachingProjection`, `CallProjection`, `MeaningView`, `ExplanationNode`, `WhyNotEntry`, `PedagogicalRelationEntry`, `PedagogicalRelationKind`, `ConventionContribution`, `SeatRelativeHandSpaceSummary` — teaching-optimized views for "why not X?" UI |
| `pedagogical-relations.ts` | `PedagogicalRelation` — discriminated union for pedagogical relations between bids/meanings |
| `predicate-surfaces.ts` | `AuctionPatternIR`, `PublicGuardIR`, `HandPredicateIR`, `DealConstraintIR`, `FullDealPredicateIR` — site-specific typed predicate surfaces scoped by evaluation world |
| `evidence-bundle.ts` | `ConditionEvidenceIR` (canonical evidence type: `conditionId`, `satisfied`, `factId?`, `observedValue?`, `threshold?`), `RejectionEvidence`, `AlternativeEvidence`, `EvidenceBundleIR` — evidence types for decision program representations. `ConditionEvidenceIR` is the single evidence type used across provenance, teaching, and evidence bundles. |
| `posterior.ts` | `PosteriorFactor`, `LikelihoodModel`, `PublicHandSpace` (with optional `latentBranches`), `SeatPosterior`, `PosteriorFactRequest`, `PosteriorFactValue`, `SubjectRef`, `PosteriorSourceRef`, `EvidenceGroupId`, `BeliefView` (enriched: `beliefId`, `subject`, `constraint`, `provenance`, `explanationKey`, `evidenceGroupId` — all optional), `LatentBranchAlternative`, `LatentBranchSet`, `PosteriorFactProvider`, `PosteriorEngine` — posterior engine contract types |
| `agreement-module.ts` | `ActivationKind`, `ModuleKind`, `PriorityClass`, `DecisionSurfaceIR`, `AttachmentIR`, `FactConstraintIR`, `PublicConsequences`, `ChoiceClosurePolicy`, `ClosureDomain`, `SystemProfileIR`, `ModuleEntryIR`, `ConflictPolicyIR`, `DeclaredEncoderKind`, `EncodingTraceIR`, `PublicEvent`, `PublicConstraint` — agreement module IR types |
...
| **Arbitration / transform vocabulary** | `contracts/meaning.ts` for `RankingMetadata` (`recommendationBand`, `specificity`, `modulePrecedence`, `intraModuleOrder`). `contracts/tree-evaluation.ts` for ranking DTOs (`ResolvedCandidateDTO`, `CandidateSet`). | The selector reads these as scalars, never as raw fact names. |
