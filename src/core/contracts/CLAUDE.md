...
| `meaning-surface.ts` | `MeaningSurfaceClause`, `MeaningSurface` (with optional `alert: "alert" \| "announce"`, `closurePolicy`) — authored, hand-independent meaning units (pre-evaluation). Public constraints auto-derived from primitive/bridge-observable clauses via `derivePublicConstraints()`. Evaluated by `conventions/core/pipeline/meaning-evaluator.ts` into `MeaningProposal`. |
