...
| `fact-catalog.ts` | `FactLayer`, `EvaluationWorld`, `FactMetadata`, `FactDefinition` (with `derivesFrom`, optional `metadata`), `FactValue`, `EvaluatedFacts`, `getFactValue()`, `FactEvaluatorFn`, `RelationalFactEvaluatorFn`, `FactCatalog` (with optional `relationalEvaluators`, optional `posteriorEvaluators`), `FactCatalogExtension`, `createFactCatalog()`, `PRIMITIVE_FACTS` (6), `BRIDGE_DERIVED_FACTS` (7, including 4 relational: `supportForBoundSuit`, `fitWithBoundSuit`, `shortageInSuit`, `totalPointsForRaise`), `SHARED_FACTS` (18 = primitive + bridge-derived + posterior-derived), `SEED_FACTS` (deprecated alias for `SHARED_FACTS`) |
| `semantic-classes.ts` | `BRIDGE_SEMANTIC_CLASSES` (platform-owned canonical IDs), `SEMANTIC_CLASSES` (deprecated compat re-export including convention-specific IDs), `SemanticClassKey`. Convenience catalog, not a gating registry — modules declare their own semantic class IDs as strings. |
...
| **Arbitration / transform vocabulary** | `contracts/meaning.ts` for `RankingMetadata` (`recommendationBand`, `specificity`, `modulePrecedence`, `intraModuleOrder`). `contracts/evaluation-dtos.ts` for ranking DTOs (`ResolvedCandidateDTO`, `CandidateSet`). | The selector reads these as scalars, never as raw fact names. |
