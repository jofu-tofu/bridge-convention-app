...
| `wasm-service.ts` | `BridgeService` — thin proxy implementing ServicePort via `wasm-bindgen` calls to Rust `WasmServicePort`. `init()` handles WASM module loading + DDS solver wiring. `createDrillSession` and `startDrill` route thrown errors through `liftServiceError` so typed Rust variants surface as TS error classes (e.g. `DealGenerationExhaustedError`). |
| `service-errors.ts` | Typed errors lifted from the WASM boundary. Rust serializes `ServiceError::DealGenerationExhausted` as JSON `{ kind: "dealGenerationExhausted", witnessSummary }` on `JsError.message`; `liftServiceError(err)` parses it into `DealGenerationExhaustedError`. `isDealGenerationExhausted(err)` is the type-guard. Other `ServiceError` variants keep the plain `Display` message — they are not catchable by discriminant. |
...
| `auth.ts` | DataPort boundary for `/api/*` calls. `DataPort` interface covers auth + billing + drill CRUD (`listDrills`, `createDrill`, `updateDrill`, `deleteDrill`, `markDrillLaunched`). `DataPortClient` is production HTTP; `DevDataPort` (dev-only) fakes auth + billing client-side and stores drills in `localStorage` under `bridge-app:dev-drills` (separate from the anonymous-mode key `bridge-app:drills`) so local dev runs without bridge-api. Drill 4xx responses are mapped to typed errors `DrillEntitlementError` (403 convention_locked), `DrillUnknownModuleError` (400 unknown_module), `DrillNotFoundError`, `DrillUnauthenticatedError`. Exports `AuthUser`, `SubscriptionTier`, `DrillDto`, `DrillCreatePayload`, `DrillUpdatePayload`. `DevDataPort` is client-only; for a real server session in e2e use `?e2e=1` (routes through `DataPortClient`) plus `POST /api/dev/login` (bridge-api `dev-tools` feature) via `tests/e2e/helpers.ts:devLogin`. |
