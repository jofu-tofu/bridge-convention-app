...
|---|---|---|---|
| Reference source | Per-convention authority in fixture `references.authority`, bridgebum for discovery only | Single global primary (bridgebum); three-tier primary/discovery/secondary | Different conventions have different authoritative sources. Larry Cohen co-invented Bergen Raises — his site is more authoritative than bridgebum. ACBL SAYC booklet defines standardized conventions. Bridgebum compiles from other sources and may diverge on thresholds. One authority = one source of truth — secondary sources were removed to eliminate ambiguity. Authority is stored in each module's fixture JSON for easy agent discovery. |
