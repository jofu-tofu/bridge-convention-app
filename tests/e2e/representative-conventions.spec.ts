...
test.describe("representative convention flows", () => {
  test("Jacoby Transfers reaches review after the seeded responder path", async ({ page }) => {
    ...
    await expect(page.getByTestId("bid-3NT")).toBeEnabled({ timeout: 5_000 });

    await page.getByTestId("bid-3NT").click();
    ...
    await expect(biddingReview).toContainText("3NT");
  });
