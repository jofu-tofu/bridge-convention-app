...
mod tests {
    ...
    fn bidding_can_go_to_three_phases() {
        assert!(is_valid_transition(GamePhase::Bidding, GamePhase::DeclarerPrompt));
        assert!(is_valid_transition(GamePhase::Bidding, GamePhase::Playing));
        assert!(is_valid_transition(GamePhase::Bidding, GamePhase::Explanation));
    }
    ...
    fn declarer_prompt_transitions() {
        assert!(is_valid_transition(GamePhase::DeclarerPrompt, GamePhase::Playing));
        assert!(is_valid_transition(GamePhase::DeclarerPrompt, GamePhase::Explanation));
        assert!(!is_valid_transition(GamePhase::DeclarerPrompt, GamePhase::Bidding));
    }

    #[test]
    fn playing_only_goes_to_explanation() {
        assert!(is_valid_transition(GamePhase::Playing, GamePhase::Explanation));
        assert!(!is_valid_transition(GamePhase::Playing, GamePhase::Bidding));
        assert!(!is_valid_transition(GamePhase::Playing, GamePhase::DeclarerPrompt));
        assert!(!is_valid_transition(GamePhase::Playing, GamePhase::Playing));
    }

    #[test]
    fn explanation_only_goes_to_declarer_prompt() {
        assert!(is_valid_transition(GamePhase::Explanation, GamePhase::DeclarerPrompt));
        assert!(!is_valid_transition(GamePhase::Explanation, GamePhase::Bidding));
        assert!(!is_valid_transition(GamePhase::Explanation, GamePhase::Playing));
        assert!(!is_valid_transition(GamePhase::Explanation, GamePhase::Explanation));
    }
}
