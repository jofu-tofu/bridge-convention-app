...
/// Intermediate state passed between phases of the build algorithm.
struct CollectedModuleData<'a> {
    modules: Vec<&'a bridge_conventions::types::module_types::ConventionModule>,
    phase_map: HashMap<String, HashMap<String, Vec<ModulePhaseState>>>,
    transitions: HashMap<String, Vec<TransitionEntry>>,
}

/// Phase 1: Resolve all modules in the bundle and collect their FSM data.
fn collect_all_module_data(
    member_ids: &[String],
    system: BaseSystemId,
) -> Option<CollectedModuleData<'_>> {
    // SAFETY: The modules are borrowed from the global registry which has 'static lifetime.
    // We use unsafe to express this lifetime relationship that Rust cannot infer.
    let modules: Vec<&bridge_conventions::types::module_types::ConventionModule> = member_ids
        .iter()
    ...
    let mut phase_map: HashMap<String, HashMap<String, Vec<ModulePhaseState>>> = HashMap::new();
    let mut transitions: HashMap<String, Vec<TransitionEntry>> = HashMap::new();

    for module in &modules {
        let (pm, trans) = collect_module_data(module, system);
        phase_map.insert(module.module_id.clone(), pm);
        transitions.insert(module.module_id.clone(), trans);
    }

    Some(CollectedModuleData {
        modules,
        phase_map,
        transitions,
    })
...
/// Phase 2: Find the root module (the one with an opener surface at the initial phase)
/// and build its subtree including R1 responder surfaces.
fn build_root_subtree(
    data: &CollectedModuleData,
    counter: &mut NodeCounter,
) -> (Option<MutableNode>, Option<String>) {
    for module in &data.modules {
        let ph_map = match data.phase_map.get(&module.module_id) {
            Some(m) => m,
            None => continue,
        };
        ...
        let first_surface = match opener_states[0].surfaces.first() {
            Some(s) => s,
            None => continue,
        };
        ...
        // Attach R1 responder surfaces from this module
        ...
        return (Some(root), Some(module.module_id.clone()));
    }

    (None, None)
}

/// Phase 3: Attach non-root modules' subtrees under the root node.
fn attach_other_modules(
...
pub fn build_bundle_flow_tree(
    ...
    let data = collect_all_module_data(&input.member_ids, system)?;

    let mut counter = NodeCounter { value: 0 };

    // Phase 2: Build root subtree from the opening module
    let (root_node, root_module_id) = build_root_subtree(&data, &mut counter);

    let mut root_node = match root_node {
        Some(r) => r,
        None => {
            // Derive entry condition from the first module's capability
            let first_module_id = data.modules.first().map(|m| m.module_id.as_str());
    ...
    // Phase 3: Attach other modules' subtrees under root
    attach_other_modules(&mut root_node, &root_mod_id, &data, &mut counter);

    // Final pass: route-constrained surfaces
    attach_route_constrained(&mut root_node, &data.phase_map, &data.modules, &mut counter);
