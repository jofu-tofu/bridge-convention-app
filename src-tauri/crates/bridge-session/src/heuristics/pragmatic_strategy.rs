...
impl BiddingStrategy for PragmaticStrategy {
    ...
    fn suggest_bid(&self, context: &BiddingContext) -> Option<BidResult> {
        ...
        try_competitive_overcall(context).or_else(|| try_protective_double(context))
    }
...
fn try_competitive_overcall(context: &BiddingContext) -> Option<BidResult> {
    ...
    let opponents_bid = context
        .auction
        .entries
        .iter()
        .any(|e| is_opponent(e, context) && !matches!(e.call, Call::Pass));
    if !opponents_bid {
        return None;
    }
...
fn try_protective_double(context: &BiddingContext) -> Option<BidResult> {
    ...
    let n = entries.len();
    let last_three_are_pass_pattern = !matches!(entries[n - 3].call, Call::Pass)  // opponent's bid
    ...
    if is_legal_call(&context.auction, &call, context.seat) {
        return Some(BidResult {
            call,
            rule_name: None,
            explanation: format!("Protective double with {} HCP in balancing seat", eval.hcp),
            ..Default::default()
        });
    }
