...
- **Alert system.** `MeaningSurface.alert?: "alert" | "announce"` declares bridge table disclosure. `resolveAlert()` derives alert from `priorityClass`/`sourceIntent` when not explicit. The formal explanation is `publicConsequences` (same `FactConstraintIR` vocabulary as the factor graph), so alerts translate directly to public beliefs without interpretation. `annotation-producer.ts` converts `publicConstraints` to `HandInference` for Layer 1 belief updates.
